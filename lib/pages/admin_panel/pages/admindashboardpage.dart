...
class _AdminDashboardPageState extends State<AdminDashboardPage> {
    ...
    try {
      ...
      if (response.status == 'SUCCESS') {
        ...
        setState(() {
          ...
          // Filter only "Order Delivered" orders
          List<OrderList> deliveredOrders = orderdetailslistpage
              .where((order) => order.orderStatus == "Order Delivered")
              .toList();

          totalOrdersCount = deliveredOrders.length;
          print("Total Delivered Orders: $totalOrdersCount");

          int totalProducts = deliveredOrders.fold(
              0, (sum, order) => sum + (order.totalProduct ?? 0));
          print("Total Delivered Products: $totalProducts");

          totalOrderPrice = deliveredOrders.fold(0.0, (sum, order) {
            return sum! + double.tryParse(order.totalPrice ?? '0')!;
          });
          print("Total Delivered Order Price: $totalOrderPrice");
  ...
  void calculateTodayOrders() {
    ...
    // Filter only "Order Delivered" orders created today
    List<OrderList> todayDeliveredOrders = orderdetailslistpage.where((order) {
      return order.orderStatus == "Order Delivered" &&
          order.createdDate != null &&
          order.createdDate!.isAfter(startOfToday);
    }).toList();

    totalTodayOrdersCount = todayDeliveredOrders.length;
    int totalTodayProducts = todayDeliveredOrders.fold(
        0, (sum, order) => sum + (order.totalProduct ?? 0));
    totalTodayOrderPrice = todayDeliveredOrders.fold(0.0, (sum, order) {
      return sum! + double.tryParse(order.totalPrice ?? '0')!;
    });

    print("Today's Delivered Orders Count: $totalTodayOrdersCount");
    print("Today's Delivered Total Products: $totalTodayProducts");
    print("Today's Delivered Total Order Price: $totalTodayOrderPrice");
